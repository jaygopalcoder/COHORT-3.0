...
form.addEventListener("submit", (event)=>{
  // uncomment this when trying to illustrate event bubbling
  // console.log("Event Bubbling");
  
  console.log("Form tag is Clicked")
...
/*  here the event happens on form but it travels all way up to the it's ancestor Tags. This is called Event Bubbling. T
his happens when we remove capture = true it forces to capture event from way up to exact element where event is occurred. This is called Event Capturing. Event Bubbling is Default behaviour of a browser. */

const main = document.querySelector("main")
main.addEventListener("submit",()=>{
  console.log("Main Tag Activated");
},capture=true)
...
// pipeline code
const pipelineBtn = document.querySelector(".pipeline-show-btn");
const pipeline = document.querySelector(".pipeline");

pipelineBtn.addEventListener("click",()=>{
  let pipelineStatus = pipeline.getAttribute("data-show-pipeline");
  if (pipelineStatus === "true"){
    pipeline.style.display = "none";
    pipelineBtn.textContent = "SHOW BROWSER PIPELINE";
    pipeline.setAttribute("data-show-pipeline", "false");
  }
  else{
    pipeline.style.display = "block";
    pipelineBtn.textContent = "HIDE BROWSER PIPELINE";
    pipeline.setAttribute("data-show-pipeline","true");
  }
})